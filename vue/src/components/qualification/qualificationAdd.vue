<template>
    <div class="main-container rounded px-5 m-2" style="width: 60%">
        ...
        <form @submit.prevent="storeStream()" class="input-text py-2">
            <div class="mb-3">
                <label for="stream" class="form-label">Stream</label>
                <input v-model="qualification.stream" type="text" class="form-control" name="stream" aria-describedby="emailHelpId"
                    placeholder="Stream">
            </div>
...
export default {
    ...
    methods: {
        storeStream() {
            ...
            axios.post('http://localhost:8000/api/qualification', this.qualification)
                .then(response => {
