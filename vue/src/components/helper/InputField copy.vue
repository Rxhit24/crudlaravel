<template>
    <div v-if="question.question_type === 'checkbox'">
        ...
        <div class="form-check" v-for="option, index in question.options" :key="index">
            <input class="form-check-input" :type="question.question_type" :value="option.title" :id="question.id + '-' + option.id"
    ...
    <div v-else-if="question.question_type === 'radio'">
        <label class="form-label">{{ question.title }}</label>
        <div class="form-check" v-for="option, index in question.options" :key="index">
            <input class="form-check-input" :value="option.title" :type="question.question_type" :id="question.id + '-' + option.id" v-model="answerRadio">
            <label class="form-check-label" :for="question.id + '-' + option.id">
                {{ option.title }}
            </label>
        </div>
